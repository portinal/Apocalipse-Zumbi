...
const runRules=[
  {title:"Cidade sem energia",rule:"Geradores, painéis, lanternas e rádios têm valor extra nesta run.",bonus:"communication"},
  {title:"Horda em 24 horas",rule:"Mobilidade e abrigo são essenciais antes da chegada da horda.",bonus:"mobility"},
  {title:"Chuva e enchentes",rule:"Rotas, veículos e abrigos protegidos valem mais nesta run.",bonus:"shelter"},
  {title:"Escassez de recursos",rule:"Água, alimento e cuidados médicos sustentam a sobrevivência.",bonus:"food"}
];
const runEvents=["Blecaute: mantenha comunicação e um abrigo preparado.","Horda avistada: mobilidade e defesa ficam sob pressão.","Tempestade: recursos e abrigo viram prioridade."];
...
function view(room) { return {roomCode:room.code,zombie:room.zombie,run:room.run,players:room.players.map(({balance,inventory,withdrawn})=>({balance,inventory,withdrawn})),item:room.item,currentBid:room.currentBid,bidder:room.bidder,starter:room.starter,canGenerate:!room.item&&!room.winner&&room.run.round<room.run.limit,message:room.message,winner:room.winner}; }
...
function finish(room) {
  if(!room.players.every((player)=>player.balance===0)&&room.run.round<room.run.limit) return;
  const points=room.players.map((player)=>player.inventory.reduce((sum,item)=>sum+item.tags.reduce((score,tag)=>score+(room.zombie.weights[tag]||0)+(tag===room.run.bonus?1:0),0),0));
...
io.on("connection",(socket)=>{
  ...
  socket.on("generate-item",(data)=>onlyPlayer(socket,data,(room,player)=>{if(player!==0)return socket.emit("action-error","Somente quem criou a sala gera o próximo item.");if(room.item||room.winner||room.run.round>=room.run.limit)return socket.emit("action-error","Esta run já terminou.");const availableItems=items.filter((item)=>!room.usedItemNames.includes(item.name));if(!availableItems.length)return socket.emit("action-error","Todos os itens desta sala já foram usados.");room.item={...availableItems[Math.floor(Math.random()*availableItems.length)]};room.usedItemNames.push(room.item.name);room.currentBid=0;room.bidder=null;room.players.forEach((entry)=>entry.withdrawn=false);const brokePlayer=room.players.findIndex((entry)=>entry.balance===0);if(brokePlayer!==-1){const buyer=1-brokePlayer;room.players[brokePlayer].withdrawn=true;room.bidder=buyer;room.currentBid=1;room.message=`Jogador ${brokePlayer+1} está sem saldo e só pode desistir. Jogador ${buyer+1} deve pegar o item por R$ 1.`;}else room.message=`Item gerado. Jogador ${room.starter+1} deve iniciar com um lance de R$ 1.`;send(room);}));
  socket.on("place-bid",(data)=>onlyPlayer(socket,data,(room,player)=>{const amount=Number(data.amount),other=1-player;if(!room.item||room.winner||room.players[player].withdrawn||room.players[other].withdrawn||room.bidder===player)return socket.emit("action-error","Esse lance não é permitido agora.");if(room.players[player].balance===0)return socket.emit("action-error","Você está sem saldo e só pode desistir deste item.");if(room.bidder===null&&player!==room.starter)return socket.emit("action-error",`O Jogador ${room.starter+1} deve iniciar este leilão.`);if(!Number.isInteger(amount)||amount<Math.max(1,room.currentBid+1)||amount>room.players[player].balance)return socket.emit("action-error","O lance precisa ser inteiro, maior que o atual e caber no saldo.");room.currentBid=amount;room.bidder=player;room.message=`Jogador ${player+1} deu R$ ${amount}. O outro jogador pode cobrir ou desistir.`;send(room);}));
