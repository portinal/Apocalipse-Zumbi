...
io.on("connection",(socket)=>{
  ...
  socket.on("claim-item",(data)=>onlyPlayer(socket,data,(room,player)=>{const other=1-player;if(!room.item||room.winner||room.bidder!==player||!room.players[other].withdrawn)return socket.emit("action-error","Você ainda não pode pegar este item.");room.players[player].balance-=room.currentBid;room.players[player].inventory.push({...room.item,price:room.currentBid});room.item=null;room.currentBid=0;room.bidder=null;room.players.forEach((entry)=>entry.withdrawn=false);room.message=`Item entregue ao Jogador ${player+1}.`;finish(room);send(room);}));
});
